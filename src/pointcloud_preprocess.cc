...
void PointCloudPreprocess::AviaHandler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const uint &i) {
        if ((msg->points[i].line < num_scans) &&
            ...
            if (i % point_filter_num == 0) {
                ...
                cloud_full[i].curvature =
                    msg->points[i].offset_time /
                    float(1000000);  // use curvature as time of each laser points, curvature unit: ms

                if ((abs(cloud_full[i].x - cloud_full[i - 1].x) > 1e-7) ||
                    (abs(cloud_full[i].y - cloud_full[i - 1].y) > 1e-7) ||
                    (abs(cloud_full[i].z - cloud_full[i - 1].z) > 1e-7) &&
                        (cloud_full[i].x * cloud_full[i].x + cloud_full[i].y * cloud_full[i].y +
                             cloud_full[i].z * cloud_full[i].z >
                         (blind * blind))) {
                    is_valid_pt[i] = true;
                }
            }
    ...
    for (uint i = 1; i < plsize; i++) {
        if (is_valid_pt[i]) {
            cloud_out.points.push_back(cloud_full[i]);
        }
    }
}
...
void PointCloudPreprocess::VelodyneHandler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    std::vector<double> yaw_fp(num_scans, 0.0);    // yaw of first scan point
    std::vector<float> yaw_last(num_scans, 0.0);   // yaw of last scan point
    std::vector<float> time_last(num_scans, 0.0);  // last offset time
    /*****************************************************************/
